...
def save_mat(x, filename):
    # x = x.squeeze(0)
    # scio.savemat(filename, {'rai_d64':x['rai_d64']})
    x = x.numpy()
    np.save(filename,x)
...
data_path = "D:/dataset/rai_d_64_dataset"  # define UCF-101 RGB data path
begin_frame, end_frame, skip_frame = 1, 33, 1
...
train_set = Dataset_CRNN(data_path, train_names, selected_frames, transform=None)
...
# data_size = len(train_loader.train_index)
...
for current_epoch in tqdm(range(1,num_epoch+1)):
    ...
    for batch_idx, X in enumerate(train_loader):
        videos = X.permute(0,2,1,3,4) # [-1,3,32,64,64]
        videos = videos.to(device)
        #videos = to_variable(videos)
        real_labels = torch.LongTensor(np.ones(32, dtype = int))
        fake_labels = torch.LongTensor(np.zeros(32, dtype = int))
        real_labels.requires_grad_(False)
        fake_labels.requires_grad_(False)
        ...
        if (batch_idx + 1) % 5 == 0:
            text_logger.info("Epoch [%d/%d], Step[%d], d_loss: %.4f, g_loss: %.4f, \
                              time: %4.4f" \
                             % (current_epoch, num_epoch, batch_idx+1, \
                             d_loss.data[0], g_loss.data[0],  time.time()-start_time))
        ...
        if (num_epoch + 1) % 5== 0:
            ...
            save_mat(sample_input.data.cpu(),
                     DIR_TO_SAVE + 'fake_gifs_sample_%s_%s_a.jpg' % (current_epoch))
            save_mat(gen_out.data.cpu()[0],
                     DIR_TO_SAVE + 'fake_gifs_sample__%s_%s_b.gif' % (current_epoch))

            save_mat(first_frame[0].data.cpu(), DIR_TO_SAVE + 'fake_gifs_%s_%s_a.jpg' % (current_epoch))
            save_mat(fake_videos.data.cpu()[0],
                     DIR_TO_SAVE + 'fake_gifs_%s_%s_b.gif' % (current_epoch))

            text_logger.info('Gifs saved at epoch: %d' % (current_epoch))
